...
import MarketplaceItem from '../../components/MarketplaceItem.jsx'
      ...
      label: 'Bots', content: <>
        {userbots == 'none' ? (
          <div className="flex flex-col h-full items-center justify-center">
            <p className="text-sky-500 text-center italic text-4xl font-semibold">Oops!</p>
            <p className="text-black dark:text-white text-center text-2xl font-medium">Seems like this user has no bots.</p>
          </div>
        )
          : <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4 mb-24 mt-5">
              {userbots.map((bot) => {
                return (<>{bot.approved ? <BotCard name={bot.name} id={bot.id} votes={bot.votes} avatar={bot.avatar} banner={bot.banner} shortdesc={bot.shortdesc} /> : <></>}</>)
              })}
            </div>
          </>
        }
      ...
      await axios.get('https://api.somelist.tk/find_bots?owner=' + router.query.id).then((res) => {
        ...
        setabots(<>
          ...
          >
            {abotsarray.map((bot) => {
              return (
              <div className="cursor-pointer bot-card h-auto sm:h-48 group hover:shadow-xl transition-all duration-200 relative mt-14 w-full bg-sky-900/10 rounded-lg">
                <div className="bot-bg w-full h-full absolute rounded-lg" style={{ background: bot.banner }}></div>
                <div className="p-4 relative">
                  <div className="flex flex-col sm:flex-row justify-center sm:justify-start items-center w-full sm:space-x-2 h-full -mt-14 mb-5">
                    <div className="relative flex-shrink-0 w-[76px] h-[76px] sm:ml-5 rounded-full drop-shadow-xl">
                      <div style={{ display: "block", overflow: "hidden", position: "absolute", inset: "0px", boxSizing: "border-box", margin: "0px" }}>
                        <img alt="" decoding="async" className="rounded-full w-full h-full" sizes="100vw" src={bot.avatar}
                          style={{ position: "absolute", inset: "0px", boxSizing: "border-box", padding: "0px", border: "none", margin: "auto",
                            display: "block", width: "0px", height: "0px", minWidth: "100%", maxWidth: "100%", minHeight: "100%", maxHeight: "100%" }}
                        />
                      </div>
                    </div>
                  ...
                  >
                    <button onClick={() => router.push('/bot/' + bot.id.toString())} className="w-full bg-sky-900/10 hover:bg-sky-900/50 hover:shadow-xl transition-all duration-200 cursor-pointer px-4 py-2 rounded-lg">
                      View
                    </button>
                    <button className="w-full bg-sky-900/10 hover:bg-sky-900/50 hover:shadow-xl transition-all duration-200 cursor-pointer px-4 py-2 rounded-lg">
                      Vote
                    </button>
                  </div>
                </div>
