...
import BotLayout from "../components/BotLayout.jsx";
...
function MyApp({ Component, pageProps }) {
  ...
  const commands = [
    {
      ...
      callback: (page) => {
        const pages = ["marketplace", "home", "store", "profile", "add bot", "bot addition"];
        if (pages.includes(page.toLowerCase())) {
          setmessage(`Redirecting to ${page}`);
          let redirect = "";
          if (page.toLowerCase() == "marketplace" || page.toLowerCase() == "store") {
            redirect = "store";
          } else if (page.toLowerCase() == "home") {
            redirect = "";
          } else if (page.toLowerCase() == "profile") {
            if (Cookie.get("token")) redirect = "profile/" + Cookie.get("id");
            else return setmessage("Login is required to visit this page.");
          } else if (page.toLowerCase() == "add bot" || page.toLowerCase() == "bot adittion") {
            if (Cookie.get("token")) redirect = "/add";
            else return setmessage("Login is required to visit this page.");
          }
          router.push(`/${redirect}`);
          setTimeout(() => setvoice(false), 1000);
        } else {
          setmessage(`Can't find page: ${page}`);
          settryagain(true);
        }
      },
    },
  ...
  useEffect(() => {
    (async () => {
      resetTranscript();
      if (voice == false) directvoice && await SpeechRecognition.startListening({ continuous: true });
    })();
  }, [voice]);
  ...
  useEffect(() => {
    ...
    if (!Cookie.get("welcome")) setIsOpen(true);
    if (Cookie.get("directVoice")) SpeechRecognition.startListening({ continuous: directvoice });
    if (Cookie.get("HighGraphics")) $("#gradient").addClass("high-graphs");
  ...
  useEffect(() => {
    if (!router.isReady) return;
    if (Cookie.get("token")) $.ajax({ url: `https://api.somelist.tk/users/notifications?token=${Cookie.get("token")}` }).then((res) => res.reply.length >= 1 && setnotifications(res.reply));
  }, [router.isReady]);

  useEffect(() => {
    setSelectedNotification(notifications[0]);
    if (notifications[0].icon != "") setNotificationsOpen(true);
  }, [notifications]);
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-center w-screen h-screen">
            <div className="relative p-6 rounded-lg bg-[#0B0A15] w-screen max-w-[800px] flex items-center justify-center flex-col">
              <AnimatePresence exitBeforeEnter>
                <motion.div key={slide} initial={{ y: 10, scale: 0.7, opacity: 0 }} animate={{ y: 0, scale: 1, opacity: 1 }} exit={{ scale: 0.7, opacity: 0 }} transition={{ duration: 1 }}>
                  {slide == 0 ? (
                    <div className="w-full flex items-center justify-center flex-col">
                      ...
                      <div className="w-full text-right mt-6">
                        <button className="py-3 px-6 text-lg rounded-lg bg-red-600 hover:bg-red-500" onClick={() => {
                          Cookie.set("welcome", true);
                          setIsOpen(false);
                        }}>Close</button>
                        <button className="py-3 px-6 text-lg rounded-lg bg-sky-600 hover:bg-sky-500" onClick={() => setslide((prevcount) => prevcount + 1)}>
                          Next
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="w-full flex items-center justify-center flex-col">
                      ...
                      <div className="w-full flex mt-6">
                        <button className="py-3 px-6 text-lg rounded-lg bg-gray-600 hover:bg-gray-500" onClick={() => setslide((prevcount) => prevcount - 1)}>
                          Previous
                        </button>
                        <button className="py-3 ml-auto px-6 text-lg rounded-lg bg-sky-600 hover:bg-sky-500" onClick={() => setslide((prevcount) => prevcount + 1)}>
                          Next
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="w-full flex items-center justify-center flex-col">
                      ...
                      <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1 }}>
                        <h1 className="mt-4 text-3xl font-semibold flex items-center">
                          <span>Edit your profile</span> <span className="ml-4 bg-sky-600 py-2 px-3 text-lg rounded-full text-white">BETA</span>
                        </h1>
      ...
      <AnimatePresence>
        {notificationsOpen && (
          <div className={"w-screen h-screen"}>
            <motion.div key="notifsbackdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div style={{ zIndex: "100" }} className={"flex left-0 top-0 fixed items-center justify-center w-screen h-screen bg-[#0B0A15]/20 backdrop-blur-lg"}>
                <motion.div key="notifsmodal" initial={{ opacity: 0, y: -300 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 500 }} className="w-screen max-w-[800px]">
                  <div className="w-screen p-6 max-w-[800px] flex items-center justify-center flex-col">
                    ...
                    <div className="w-full flex mt-6">
                      <button className="py-3 ml-auto px-6 text-lg rounded-lg bg-sky-600 hover:bg-sky-500" onClick={() => {
                        ...
                        if (selectedNotifNumber + 1 == notifications.length) setNotificationsOpen(false);
                        else setSelectedNotification(notifications[selectedNotifNumber]);
                      }}>Next</button>
                    </div>
                  </div>
                </motion.div>
      ...
      <div id="gradient"></div>
      {process.env.NEXT_PUBLIC_LOADING == "true" && router.pathname != "/404" && (
          ...
          <motion.div key={"voicedialog"} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="w-screen h-screen bg-gray-900/50 py-10 flex-col backdrop-blur-xl fixed flex items-center" style={{ zIndex: 101, top: 0, left: 0 }}>
              <h1 className="text-4xl text-white/80">
                <i className="fad fa-microphone text-5xl mr-4" />
                Voice Control
              </h1>
              ...
              {listening ? (
                <>
                  ...
                  <button
                    className="bg-red-600 rounded-full w-14 h-14 text-2xl mt-4"
                ...
                <>
                  {message ? (
                    <>
                      ...
                      {help && (
                        <ol>
                          <li>Help - See this command list</li>
                          <li>Open [page] - Navigates to a page. (Avaliable pages: Home, Marketplace, Profile)</li>
                          <li>Add a robot - Navigates to the Add Bot page</li>
                          <li>Look for [bot_name] - Navigates to the homepage and interacts with the search input.</li>
                        </ol>
                      )}
                    </>
                  ) : (
                    <>
                      ...
                      <button
                        className="bg-sky-600 rounded-full w-14 h-14 text-2xl mt-4"
                        onClick={() => {
