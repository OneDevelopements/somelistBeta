...
export default function Add({ isLoggedIn }) {
    ...
    const nextstage = () => {
        if (stagecount == 1) return setstagecount(stagecount + 1);
        ...
        if (stagecount == 2) {
            ...
            $.ajax({
                ...
                if (res.result == 'INVALID') {
                    ...
                    setloading(false)
                    setstagecount(1)
                    return toast.warning('This bot has already been added to Somelist.', {
                        autoClose: 3000,
                        closeOnClick: true,
                        draggable: true
                    });
    ...
    const submitbot = () => {
        ...
        let soptions = []
        selectedOptions.map((option) => soptions.push(option.label))
    ...
    return (
        <>
            ...
            <form method='POST' onSubmit={(e) => submitbot(e)}>
                <div style={{ position: 'fixed', bottom: '0', zIndex: '99' }} className="px-10 flex backdrop-blur-md bg-[#E3E5E8] dark:bg-[#080712] bg-opacity-50 w-screen p-4">
                    <div className="" style={{ marginLeft: '-15px' }}>
                        <button className={`opacity-100 h-14 bg-gradient-to-br from-gray-800 to-gray-800 py-4 px-6 flex items-center rounded-lg text-white shadow-sm shadow-sky-slate/20`}
                            onClick={() => stagecount != 1 && setstagecount(stagecount - 2)} disabled={stagecount <= 2 && true} type='button'>
                            <i className="fas fa-arrow-left"></i> <p className="font-semibold ml-1" >Previous</p>
                        </button>
                    </div>
                    <div className="flex w-full justify-center text-center items-center">{step}</div>
                    <div>
                        <button className="opacity-100 h-14 bg-gradient-to-br from-sky-600 to-sky-800 py-4 px-6 flex items-center rounded-lg text-white shadow-sm shadow-sky-600/20"
                            onClick={() => { stagecount == 5 ? submitbot() : nextstage() }} disabled={loading} type='button'>
                ...
                <div class="p-5 lg:p-10 py-[8rem] lg:py-[10rem] rounded-lg min-h-screen">
                    ...
                    <div className={stagecount != 3 && 'hidden'}>
                        ...
                        <div className='lg:flex my-32 items-center'>
                            ...
                            <Select value={selectedOptions} styles={customStyles} options={options} isMulti closeMenuOnSelect={false} className='w-full' onChange={(e) => setSelectedOptions(e)} />
                        </div>
                        <div className='lg:flex my-32 items-center'>
                            <div className="mr-6 mb-3 w-full lg:w-60 lg:mb-0">
